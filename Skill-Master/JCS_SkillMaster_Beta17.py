...
# Beta 17: BUILD GO mode trains every automatable build skill to its saved goal using the smart scheduler.
...
BUILTIN_BUILDS = {
    "Lute Skywalker - Bard Tamer": {
        ...
        "note": "Discord-trained pet bard/tamer preset. Musicianship is free on InsaneUO.",
    },
    "Basher - Poison": {
        "skills": {
...
class FreeSkillsMaster:
    ...
    def _current_build_payload(self, name=""):
        return {
            ...
            "stats": {k: float(self.stat_goals.get(k, 0.0) or 0.0) for k in ("str","dex","int") if self._stat_goal(k) > 0},
        }
    ...
    def _apply_build_payload(self, payload, replace=True):
        ...
        if isinstance(stats, dict):
            for st in ("str","dex","int"):
                try:
                    if st in stats:
                        self.stat_goals[st] = max(0.0, float(stats[st]))
                except Exception:
                    pass
        self.save()
    ...
    def _safe_build_filename(self, name):
        safe = re.sub(r"[^A-Za-z0-9_-]+", "_", str(name or "Build")).strip("_") or "Build"
        return BUILD_SHARE_PREFIX + safe[:48] + ".json"

    def export_current_build(self, name=None):
        name = str(name or pget("LastBuildName", "Shared Build") or "Shared Build")
        payload = self._current_build_payload(name)
        try:
            base = os.path.dirname(os.path.abspath(__file__))
        except Exception:
            base = "."
        path = os.path.join(base, self._safe_build_filename(name))
        try:
            with open(path, "w") as f:
                json.dump(payload, f, indent=2, sort_keys=True)
            self.status = "Build exported: {}".format(os.path.basename(path))
            self.build_ui()
            return True
        except Exception as e:
            self.status = "Build export failed: {}".format(str(e)[:45])
            self.build_ui()            return False

    def _shared_build_files(self):
        try:
            base = os.path.dirname(os.path.abspath(__file__))
            names = []
            for fn in os.listdir(base):
                if fn.startswith(BUILD_SHARE_PREFIX) and fn.lower().endswith(".json"):
                    names.append(fn)
            return sorted(names)[:30]
        except Exception:
            return []

    def import_shared_build(self, filename):
        try:
            base = os.path.dirname(os.path.abspath(__file__))
            path = os.path.join(base, filename)
            with open(path, "r") as f:
                payload = json.load(f)
            if self._apply_build_payload(payload, replace=True):
                self.status = "Imported shared build: {}".format(payload.get("name", filename))
            else:
                self.status = "Shared build contained no recognized skills."
        except Exception as e:
            self.status = "Import failed: {}".format(str(e)[:50])
    ...
    def _stat_goal(self, stat):
        try:
            return max(0.0, float(self.stat_goals.get(stat, 0.0) or 0.0))
        except Exception:
            return 0.0
    ...
    def _stat_goal_total(self):
        return sum(self._stat_goal(st) for st in ("str", "dex", "int") if self._stat_goal(st) > 0.0)

    def _required_stat_cap(self):
        desired = self._stat_goal_total()
        current_cap = self._stat_total_cap()
        if desired <= current_cap + 0.5:
            return 0
        return int(((desired + 4.999) // 5) * 5)
    ...
    def open_stat_editor(self, stat):
        ...
        title = API.CreateGumpTTFLabel("SET {} GOAL".format(stat.upper()),14,C_TITLE,"","center",310,False)
        title.SetRect(10,8,310,22); g.Add(title)
        box = API.CreateGumpTextBox(str(int(self._stat_goal(stat))) if self._stat_goal(stat) else "0",120,30,False,16)
        ...
        help_text="0 = leave locked | below goal = UP | above goal = DOWN"
        if cap_need:
    ...
    def save_stat_editor(self):
        ...
        self.stat_goals[stat]=max(0.0,value)
        self.save()
        self.close_stat_editor()
        if self.auto_build_locks: self.apply_build_locks(silent=True)
        self.status="{} goal set to {:.0f}.".format(stat.upper(),self._stat_goal(stat))
        self.build_ui()
    ...
    def _build_build_page(self):
        ...
        self._button("BUILDS", 80, 120, 70, self.open_build_library, h=28,
                     tooltip="Load built-in, saved, or shared build files.")
        self._button("SAVE", 156, 120, 58, self.open_build_name_editor, h=28,
                     tooltip="Save this build by name for later use.")
        self._button("SHARE", 220, 120, 62, self.export_current_build, h=28,
                     tooltip="Export the current build as a shareable JCS_Build_*.json file.")
        self._button("AUTO LOCK ON" if self.auto_build_locks else "AUTO LOCK OFF", 288, 120, 112,
        ...
        self._button("STOP" if self.build_training else "GO", 406, 120, 68,
                     self.toggle_build_training, h=28, state=self.build_training,
                     tooltip="GO trains every automatable skill in THIS build toward its saved goal. "
                             "Manual-only skills remain on the checklist. Skills needing SETUP wait until setup is complete. "
                             "Skills at their current cap wait for the required Power Scroll. Press STOP to halt build training.")

        stat_x = 482
        for i, stat in enumerate(("str","dex","int")):
            x = stat_x + i*104
            cur = self._stat_value(stat); goal = self._stat_goal(stat); state = self._stat_state_for_goal(stat).upper()
            stat_cap_needed = self._required_stat_cap()
            self._panel(x, 116, 100, 50, C_PANEL_2)
            self._label("{} {:.0f}".format(stat.upper(), cur), x+4, 120, 42, C_TEXT, 9)
            stat_tip = "Set {} target. Current lock plan: {}. Total stat goals {:.0f}; current stat cap {:.0f}.".format(stat.upper(), state, self._stat_goal_total(), self._stat_total_cap())
            if stat_cap_needed:
                stat_tip += " This build requires at least a {} total Stat Cap Scroll.".format(stat_cap_needed)
            self._button("G:{:.0f}".format(goal), x+46, 120, 50, lambda st=stat: self.open_stat_editor(st), h=22,
                         state=True if goal and abs(cur-goal)<=0.5 else None, tooltip=stat_tip)
            stat_state_text = "NEEDS {} CAP".format(stat_cap_needed) if stat_cap_needed else state
            self._label(stat_state_text, x+4, 143, 92, C_GREEN if state=="LOCKED" and not stat_cap_needed else C_GOLD, 7 if stat_cap_needed else 8, "center")

        self._label("BUILD SKILLS", 18, 180, 160, C_ORANGE, 12)
        self._label("CURRENT", 286, 180, 70, C_ORANGE, 11, "center")
        self._label("GOAL", 366, 180, 60, C_ORANGE, 11, "center")
        self._label("STATUS", 438, 180, 100, C_ORANGE, 11, "center")
        self._label("TRAINER", 548, 180, 82, C_ORANGE, 11, "center")
        self._label("LOCK", 638, 180, 72, C_ORANGE, 11, "center")
        self._label("EDIT", 718, 180, 62, C_ORANGE, 11, "center")
        y=204
        if not skills:
            self._panel(18,y,772,54)
            self._label("No build loaded. Click ADD SKILL, choose a skill, then click its GOAL to set the desired level.",32,y+16,740,C_MUTED,12,"center")
        else:
            for skill in skills[:9]:
                ...
                self._label(skill,28,y+8,240,C_TEXT,12)
                self._label("{:.1f}".format(val),286,y+8,70,C_GREEN if done else C_GOLD,11,"center")
                ...
                self._button(self._goal_text(skill),366,y+6,60,lambda s=skill:self.open_goal_editor(s),h=26,state=True if done else None,tooltip=goal_tip)
                status_text = "DONE" if done else (("NEEDS {} PS".format(ps_needed)) if ps_needed else "NEEDS {:.1f}".format(max(0.0,goal-val)))
                self._label(status_text,438,y+8,100,C_GREEN if done else C_GOLD,9 if ps_needed else 10,"center")
                trainer = ("AUTO 90+" if skill == "Animal Taming" else ("AUTO" if skill in AUTOMATED_SKILLS else "MANUAL"))
                self._label(trainer,548,y+8,82,C_GREEN if skill in AUTOMATED_SKILLS else C_MUTED,10,"center")
                lock_text = "LOCK" if done else ("WAIT PS" if train_blocked else "UP")
                self._label(lock_text,638,y+8,72,C_GREEN if done else C_GOLD,9,"center")
                self._button("REMOVE",714,y+6,66,lambda s=skill:self.remove_build_skill(s),h=26,
        ...
        self._panel(12,646,796,40,C_PANEL_2)
        self._label("Channel scheduler overlaps compatible CAST / TARGET / SELF / TOOL work. BUILDS can be saved and shared as JSON.",20,656,780,C_MUTED,10,"center")
    ...
    def _setup_tooltip(self, skill):
        tips = {
            ...
            "Peacemaking": "NO CREATURE SETUP REQUIRED: Skill Master trains with AREA PEACE by targeting yourself. Instruments are auto-detected in your backpack and automatically replaced when they wear out.",
            ...
            "Animal Taming": "SETUP (90+ TAMING): First train Animal Taming manually to at least 90.0 and activate the Animal Taming mastery that provides Combat Training. Then click one nearby controlled pet. Skill Master repeatedly casts Combat Training, targets that pet, closes the mastery result gump, and repeats until your Animal Taming GOAL is reached. Keep the pet nearby and visible.",
        }
    ...
    def open_build_library(self):
        ...
        y+=8; label("SHARED FILES IN SCRIPT FOLDER",18,y,330,C_ORANGE,12); y+=24
        shared=self._shared_build_files()
        if not shared: label("No JCS_Build_*.json files found.",22,y,340,C_MUTED,10); y+=24
        for fn in shared[:6]:
            btn(fn[:42],20,y,330,lambda f=fn:self.import_shared_build(f)); y+=28
    ...
    def _substatus(self, skill):
        ...
        if skill in BARD_SKILLS:
            ...
            if skill == "Peacemaking":
                return "Area Peace -> SELF | auto instrument | gains: {}".format(self.gain_counts.get(skill, 0))
        ...
        if skill == "Animal Taming":
            ...
            if value < 90.0:
                return "Manual training to 90.0 required | currently {:.1f}".format(value)
            if not pet:
                return "90+ ready | needs SETUP pet for Combat Training mastery"
            return "Combat Training pet 0x{:X} | gains: {}".format(pet, self.gain_counts.get(skill, 0))
    ...
    def _train_bard_skill(self, skill):
        ...
        try:
            ...
            if not API.WaitForTarget("any", TARGET_TIMEOUT):
                return False, "no instrument target cursor"
            ...
            # Prefer a self-target whenever the skill supports it. Area Peace is
            # much more reliable for unattended training than maintaining an
            # outside creature target.
            if skill == "Peacemaking":
                API.Target(int(API.Player.Serial))
            else:
                API.Target(first)
    ...
    def _train_action(self, skill):
        ...
        if skill in ("Fishing", "Mining", "Lumberjacking"):
            ...
            if not (int(x) or int(y)):
                return False, "no practice spot"            if has_target():
                return False, "target cursor busy"
    ...
    def train_tick(self):
        ...
        candidates=[]
        for skill in ALL_SKILLS:
            if skill not in AUTOMATED_SKILLS: continue
            if not self._effective_enabled(skill): continue
            if self._at_goal(skill): self.enabled[skill]=False; continue
            if self._at_training_limit(skill): continue
            if not self._ready(skill): continue
        ...
        for skill in ordered:
            ...
            if ok:
                ...
                if recovering_cast and skill not in CASTING_SKILLS:
