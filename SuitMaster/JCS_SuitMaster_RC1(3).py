...
# RELEASE CANDIDATE 1 - PUBLIC TEST BUILD
# ============================================================
...
def classify_slot(item, opl):
    ...
    # Name-first exceptions for InsaneUO artifact layers that may share generic
    # Legion layer names.
    if "sash" in name or "tunic of vigor" in name:
        return "Sash"
    if "belt" in name or "cincture" in name:
        return "Belt"
    if "earring" in name or "earrings" in layer:
        return "Earrings"
    if "cloak" in name or "dragon's wing" in name or "dragons wing" in name:
        return "Back"
    if "robe" in name or "epaulette" in name:
        return "Robe"

    # Direct Legion layer names first.
    layer_map = {
        "head": "Head",
        "helmet": "Head",
        "neck": "Neck",
    ...
    # Shields often report a hand layer.
    shield_words = (
    ...
    if "belt" in name or "cincture" in name:
        return "Belt"
    if "cloak" in name or "wing" in name:
        return "Back"
    if "robe" in name or "epaulette" in name:
        return "Robe"
    ...
    if any(x in name for x in ("helmet", "helm", "cap", "hat", "cowl", "circlet")):
        return "Head"
    if any(x in name for x in ("gorget", "necklace")) and "necklace" not in name:
        return "Neck"
    if any(x in name for x in ("gloves", "gauntlets", "mitts")):
        return "Hands"
    if any(x in name for x in ("sleeves", "arms")):
        return "Arms"
    if any(x in name for x in (
        "tunic", "chest", "breastplate", "robe", "armor", "armour",
        "jacket", "shirt"
    )):
        # Avoid swallowing known non-chest armor names.
        if not any(x in name for x in ("leggings", "leg armor", "leg armour")):
            return "Chest"
...
def prune_candidates(slot_items, profile, limit=30):
    # Keep the most promising items per slot. This prevents combinatorial
    # explosion while preserving varied high-quality candidates.
    ranked = sorted(
        slot_items,
...
def score_totals(totals, profile):
    ...
    # Hard minimums dominate the solution.
    for p, minimum in reqs.items():
    ...
    # Preferences contribute until their configured target.
    for p, weight in weights.items():
        w = int(weight or 0)
        if w <= 0:
            continue
    ...
    if budget > 0:
        ...
        score += (float(reached) / float(budget)) * 10000.0
...
def item_rough_score(item, profile):
    ...
    for p, w in profile.get("weights", {}).items():
        ...
        score += min(value, target) * int(w or 0) * 10.0
...
def show_profile_manager():
    ...
    add_label(
        g,
        "Share codes include the COMPLETE profile: mins, targets, priorities, shield rule, skill budget and scan metadata. No item/chest serials.",
...
def show_profile_editor(profile_name, section_name=None):
    ...
    add_label(g, "SECTION", 20, 100, 80, 22, 11, C_GOLD)
    section_dd = API.CreateDropDown(235, section_order, section_order.index(section_name))
    section_dd.SetPos(100, 96)
    g.Add(section_dd)

    add_label(g, "Edit one page at a time. OPEN PAGE keeps edits in memory. Skill pts = flexible TOTAL +skill required from gear.",
              355, 99, 455, 38, 10, C_MUTED)

    add_panel(g, 16, 140, 808, 390)
    add_label(g, section_name, 32, 154, 280, 22, 13, C_TITLE)
    add_label(g, "PROPERTY", 32, 184, 300, 20, 11, C_GOLD)
    add_label(g, "MIN", 470, 184, 55, 20, 11, C_GOLD)
    add_label(g, "TARGET", 555, 184, 70, 20, 11, C_GOLD)
    add_label(g, "PRIORITY", 655, 184, 90, 20, 11, C_GOLD)
...
def optimize():
    ...
    for raw_score, chosen, totals in beam:
        ...
        ranked.append((final_score, raw_score, chosen, totals))
    ...
    if swing and not swing.get("meets_max_swing", True):
        set_status(
