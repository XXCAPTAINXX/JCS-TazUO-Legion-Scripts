...
class UltimateCombatBar:
    def __init__(self):
        ...
        # Enemy of One should persist across different serials of the same
        # creature type. Store the last type we armed it for so we only
        # cancel/recast when combat changes to a different kind of mob.
    ...
    def _label(self, text, x, y, width, color="#FFFFFF", size=12, align="left"):
        label = API.Gumps.CreateGumpTTFLabel(text, size, color, "", align, width, False)
    ...
    def _button(self, text, x, y, width, callback, height=23):
        ...
        button.SetPos(x, y)
        self.ui.Add(button)
        API.Gumps.AddControlOnClick(button, callback)

    def _background(self, width, height):
        bg = API.Gumps.CreateGumpColorBox(0.78, "#17120D")
    ...
    def _item_icon_button(self, graphic, x, y, width, height, callback):
        ...
        try:
            pic = API.Gumps.CreateGumpItemPic(int(graphic), int(width), int(height))
        except Exception:
            try:
        ...
        try:
            API.Gumps.AddControlOnClick(pic, callback)
        except Exception:
            try:
    ...
    def build_ui(self):
        # IMPORTANT:
        # Never Clear() and reuse the old custom gump when switching views.
        # In Legion, another server/container/loot gump can leave the old
        # custom surface visually alive even though our reference changes.
        #
        # We explicitly Dispose() our CURRENT J.C.S. gump before creating
        # the replacement. That guarantees Status / Setup / Minimized are
        # mutually exclusive instead of stacking on screen.
        old_ui = self.ui
        ...
        self.ui = API.Gumps.CreateGump(True, True, True)
        API.Gumps.AddGump(self.ui)
    ...
    def refresh_combat_ui(self):
        """Refresh runtime status only when it is safe to do so.

        IMPORTANT:
        While Setup is open, combat events are NOT allowed to rebuild the gump.
        This prevents external gumps (loot, corpses, containers, etc.) from
        coinciding with a combat refresh and leaving both Setup and a newly
        rebuilt status gump on screen.
        """
        if self.setup_mode:
            return
    ...
    def _build_minimized(self):
        # Compact utility bar with tight spacing and full-width controls.
        width, height = 470, 42
        ...
        # Pull status closer to the title to remove wasted horizontal space.
        ...
        # Durability quick check. Use the lowest-durability gear graphic when
        # Legion can render it; otherwise fall back to a small Dura button.
        ...
        # Restore the original compact apple spacing. Enlarging the item-pic
        # control did not enlarge the actual shard art, so keep it at 28x28.
        ...
        # Leave enough room so neither button is clipped.
    ...
    def _build_full(self):
        ...
        self._label("v1.6 RC13 PUBLIC TEST", 305, 236, 180, "#B6B0A7", 9, "right")
    ...
    def _get_send_bag_charges(self, force=False):
        """Read Bag of Sending charges from its displayed item properties."""
    ...
    def _refresh_enchanted_apples(self, force=False):
        """Find enchanted apples in the backpack and cache serial/graphic/count."""
        now = time.time()
        ...
        for item in list(items or []):
            ...
            # Some shards do not populate Item.Name until props are requested.
    ...
    def _lowest_durability_graphic(self):
        """Return the graphic for the equipped item with the lowest durability."""
        worst = None
    ...
    def check_durability(self, force_message=False):
        ...
        # "Check Now" should always tell the player which equipped item is
        # currently in the worst condition, even when it is still above the
    ...
    def _combine_gold_to_root(self):
        """Move all backpack gold to the backpack root.

        Legion's MoveItem stacks by default when no x/y are supplied. The UO
        server will keep gold stacks within its stack-size limit and create
        another stack for overflow.
        """
        ...
        # Pull nested gold stacks into the backpack root first.
        for item in gold:
            serial = self._serial_of(item)
        ...
        # Re-drop root stacks through the backpack so the client/server gets
        # a chance to consolidate compatible stacks, while the server itself
        # enforces the 60,000-gold maximum stack size.
    ...
    def send_gold(self):
        ...
        # Largest stacks first. Each Bag of Sending use sends one stack.
        stacks = sorted(
            stacks,
        ...
        for item in stacks:
            ...
            # Defensive sanity check. OSI gold stacks should never exceed this.
    ...
    def _equip_profile(self, requested):
        ...
        # Move requested weapon to backpack root first.
    ...
    def _enemy_type_key(self, enemy):
        """Return a stable Enemy-of-One grouping key for a mobile.

        Prefer body/graphic because renamed mobs of the same creature type
        should still count as the same Enemy of One target. Fall back to the
        normalized name if a graphic is unavailable.
        """
        if not enemy:
            return None
    ...
    def _manage_enemy_of_one(self, enemy):
        """Keep Enemy of One running until combat changes creature type."""
        ...
        # Same creature type and EOO is already active: leave it alone.
        if active and self.eoo_enemy_key == new_key:
            return

        # If EOO is active but we have moved to a different creature type,
        # toggle/cast it once to clear the old type before re-arming it.
        ...
        # Arm EOO for the new creature type. If the previous buff disappeared
        # naturally, this also restores it without forcing an unnecessary
        # cancel step.
    ...
    def _basher_ready(self):
        """Return True when this character can sensibly attempt Parry Shield Bash."""
    ...
    def _shield_bash(self):
        """Arm the Parrying mastery Shield Bash when available.

        UO Shield Bash costs 40 mana and triggers from a successful hit or parry.
        The shard/client must have the Parrying mastery active/unlocked.
        """
        ...
        try:
            ...
            # Experimental Basher support must never interrupt the combat loop.
            return False

    def _buffs(self):
        # Basher support: arm Parry mastery Shield Bash only when the character
        # has enough Parrying, the configured shield is actually equipped,
        # and the optional Bash toggle is enabled.
        self._shield_bash()
    ...
    def _single_target_ability(self):
        """Choose the useful single-target move for the detected weapon.

        Known pairs used here:
          Double Axe      = Double Strike / Whirlwind
          Longsword       = Armor Ignore / Concussion Blow
          Broadsword      = Crushing Blow / Armor Ignore
          Bladed Whip     = Bleed Attack / Whirlwind
          Radiant Scimitar= Whirlwind / Bladeweave
          Composite Bow   = Armor Ignore / Moving Shot
          Soul Glaive     = Armor Ignore / Moving Shot
        """
        # Swordsmanship mastery Onslaught remains the preferred debuff where
        # enabled. It is independent of the weapon's primary/secondary move.
        ...
        if self.weapon_type == "broadsword":
            # Armor Ignore is the Broadsword secondary and is generally the
            # better focused-target choice than Crushing Blow.
            if self._mana() >= 30:
        ...
        if self.weapon_type == "radiantscimitar":
            # Bladeweave is the only non-AOE special on this weapon.
            if self._mana() >= 15:
                self._activate_secondary()
            else:
                self._clear_weapon_abilities()
        ...
        if self.weapon_type in ("longsword", "compositebow", "soulglaive"):
            # Armor Ignore is primary on all three.
            if self._mana() >= 30:
        ...
        if self.weapon_type == "doubleaxe":
            # Double Strike is primary; reserve Whirlwind for crowds.
            if self._mana() >= 30:
                self._activate_primary()
            else:
                self._clear_weapon_abilities()
        ...
        if self.weapon_type == "bladedwhip":
            # Bleed Attack is primary; reserve Whirlwind for crowds.
            if self._mana() >= 30:
                self._activate_primary()
            else:
                self._clear_weapon_abilities()
    ...
    def _multiple_target_ability(self):
        """Use true multi-target attacks only when the weapon actually has one."""
        # These are the supported weapons with Whirlwind Attack.
        if self.weapon_type == "radiantscimitar":
            # Radiant Scimitar primary = Whirlwind.
            if self._mana() >= 15:
                self._activate_primary()
            else:
                self._clear_weapon_abilities()
        ...
        if self.weapon_type in ("bladedwhip", "doubleaxe"):
            # Bladed Whip and Double Axe secondary = Whirlwind.
            if self._mana() >= 15:
                self._activate_secondary()
            else:
                self._clear_weapon_abilities()
        ...
        # Broadsword has Crushing Blow / Armor Ignore. Neither is an AOE.
        # Longsword, Composite Bow and Soul Glaive likewise have no true
        # multi-target special in their primary/secondary pair. Do NOT turn
        # on a single-target special merely because the player is surrounded.
        #
        # If Momentum Strike is enabled, it is the intentional crowd fallback.
        if self.settings["momentum"] and self._mana() >= 10:
            self._clear_weapon_abilities()
            self._cast("Momentum Strike", 10, 1.0, "Momentum Strike")
            return

        # Otherwise continue focused-target logic. For a Broadsword this means
        # Armor Ignore, not Crushing Blow.
    ...
    def combat_tick(self):
        ...
        # This exact enemy drives BOTH attack logic and Slayer selection.
        self._new_target(enemy)

        # Auto AOE may override slayer weapon, but never the enemy choice.
