...
class UltimateCombatBar:
    ...
    def _save_settings(self):
        for key in DEFAULTS:
            try:
                API.SavePersistentVar("JCS_UCB_" + key, "1" if self.settings[key] else "0", API.PersistentVar.Char)
            except Exception:
                pass
        try:
            API.SavePersistentVar("JCS_UCB_WeaponType", self.weapon_type, API.PersistentVar.Char)
        except Exception:
            pass
    ...
    def _load_learned_name(self, name):
        try:
            value = API.GetPersistentVar(self._mob_key(name), "", API.PersistentVar.Char)
    ...
    def _load_learned_graphic(self, graphic):
        if not graphic:
            return None
        try:
            value = API.GetPersistentVar(self._graphic_key(graphic), "", API.PersistentVar.Char)
            return value if value in SLAYER_PROFILES else None
        except Exception:
            return None
    ...
    def _save_learning(self, name, graphic, profile):
        ...
        try:
            if name:
                API.SavePersistentVar(self._mob_key(name), profile, API.PersistentVar.Char)
            if graphic:
                API.SavePersistentVar(self._graphic_key(graphic), profile, API.PersistentVar.Char)
        except Exception:
            pass
    ...
    def _clear_current_learning(self):
        ...
        try:
            if self.current_enemy_name:
                API.RemovePersistentVar(self._mob_key(self.current_enemy_name), API.PersistentVar.Char)
        except Exception:
            pass
        try:
            if self.current_enemy_graphic:
                API.RemovePersistentVar(self._graphic_key(self.current_enemy_graphic), API.PersistentVar.Char)
        except Exception:
            pass
    ...
    def _build_minimized(self):
        ...
        if armor_graphic:
            control = self._item_icon_button(armor_graphic, 254, 5, 30, 30, lambda: self.check_durability(True))
            if control is None:
        ...
        if self.enchanted_apple_graphic:
            self._item_icon_button(self.enchanted_apple_graphic, 296, 5, 28, 28, self.use_enchanted_apple)
            self._label(str(self.enchanted_apple_count), 321, 9, 24, "#FFFFFF", 12, "center")
        else:
        ...
        if bag_graphic:
            ...
            self._label("?" if charges is None else str(charges), 373, 9, 25, "#FFD166", 12, "center")
        else:
    ...
    def _build_full(self):
        ...
        self._button("Apple [%d]" % apple_count, 267, 34, 65, self.use_enchanted_apple)
        ...
        dura_text = "LOW x%d" % len(self.low_durability) if self.low_durability else "OK"
        dura_color = "#FF7A7A" if self.low_durability else "#72E39A"
        self._label("Dura", 255, 175, 44, "#E7A65B")
        self._label("%s  <%d%%" % (dura_text, self.durability_threshold), 302, 175, 110, dura_color, 10)
        state = "PAUSED" if self.paused else "ACTIVE"
        state_color = "#FFD166" if self.paused else "#72E39A"
        self._label(state, 12, 205, 400, state_color, 11, "center")
        if not self.setup_mode:
            self._label("Setup: weapons, shield, gold bag, durability, abilities, and learning.", 12, 232, 405, "#B6B0A7", 9, "center")
            return
        self._label("SETUP", 12, 234, 100, "#E7A65B", 14)
        self._label("v1.6 RC15", 305, 236, 180, "#B6B0A7", 9, "right")
        self._label("Basher / Shield Bash support is BETA - Parry Mastery testers wanted.", 12, 254, 474, "#FFD166", 9, "center")
        self._button("Weapon Bag", 12, 278, 110, self.set_weapon_bag)
        self._button("Shield", 128, 278, 82, self.set_shield)
        self._button("Slayer Auto: ON" if self.settings["auto_slayer"] else "Slayer Auto: OFF", 216, 278, 132, self.toggle_auto_slayer)
        self._button("Auto AOE: ON" if self.settings["auto_aoe"] else "Auto AOE: OFF", 354, 278, 132, self.toggle_auto_aoe)
        ...
        setup_send_text = "Send Gold" if not self.send_bag_serial else "Send (%s)" % ("?" if setup_charges is None else str(setup_charges))
        ...
        self._button("%d%%" % self.durability_threshold, 284, 324, 58, lambda: self.check_durability(True))
        ...
        for index, profile in enumerate(SLAYER_PROFILES):
            ...
            x, y = 12 + col * 160, 382 + row * 29
            short = "Single Target" if profile == "General Single Target" else "General AOE" if profile == "General AOE" else "Poison Elem" if profile == "Poison Elemental" else profile
            marker = "* " if self.slayer_weapons.get(profile, 0) else ""
            self._button(marker + short, x, y, 152, lambda p=profile: self.assign_slayer_weapon(p), 24)
        self._label("Combat Options", 12, 504, 150, "#E7A65B", 12)
        for index, (key, name) in enumerate(ABILITY_OPTIONS):
            col, row = index % 3, index // 3
            x, y = 12 + col * 160, 526 + row * 26
            marker = "ON " if self.settings[key] else "OFF "
            display_name = {"cw":"Consecrate","momentum":"Momentum","bash":"Shield Bash BETA"}.get(key, name)
            self._button(marker + display_name, x, y, 152, lambda k=key: self.toggle_setting(k), 22)
        self._label("Teach Target Slayer Type", 12, 634, 190, "#E7A65B", 11)
        self._button("Target Mob", 300, 630, 90, self.target_enemy, 22)
        self._label("1. Target a monster   2. Choose the slayer type below", 12, 656, 470, "#E8EAF0", 11)
        learn_profiles = ["General Single Target", "Arachnid", "Demon", "Dragon", "Elemental", "Poison Elemental", "Fey", "Repond", "Reptile", "Undead"]
        for index, profile in enumerate(learn_profiles):
            x, y = 12 + (index % 3) * 160, 676 + (index // 3) * 23
            short = "Single Target" if profile == "General Single Target" else "Poison Elem" if profile == "Poison Elemental" else profile
            self._button(short, x, y, 152, lambda p=profile: self.learn_current_mob(p), 20)
        self._button("Forget Learned Target", 12, 768, 180, self._clear_current_learning, 20)
    ...
    def _get_send_bag_charges(self, force=False):
        ...
        for line in str(props).replace("\r", "").split("\n"):
            ...
            if "charge" not in lower:
                continue
            match = re.search(r"charges?\s*[:\-]?\s*(\d+)", lower) or re.search(r"(\d+)\s*charges?", lower)
            if match:
                try:
    ...
    def set_send_bag(self):
        ...
        API.SysMsg("Bag of Sending saved. Charges could not be read." if charges is None else "Bag of Sending saved with %d charge(s)." % charges, 68)
        self.build_ui()

    def adjust_durability(self, amount):
        self.durability_threshold = max(5, min(95, int(self.durability_threshold) + int(amount)))
        self._save_int("JCS_UCB_DuraWarnPct", self.durability_threshold)
        API.SysMsg("Durability warning set to %d%%." % self.durability_threshold, 68)
    ...
    def _detect_assigned_weapon_type(self, serial):
        ...
        try:
            ...
            name = ""
        if "whip" in name: return "bladedwhip"
        if "broadsword" in name: return "broadsword"
        if "double axe" in name or "doubleaxe" in name: return "doubleaxe"
        if "radiant scimitar" in name: return "radiantscimitar"
        if "longsword" in name: return "longsword"
        if "composite bow" in name: return "compositebow"
        if "soul glaive" in name: return "soulglaive"
        return ""
    ...
    def assign_slayer_weapon(self, profile):
        ...
        self._save_int("JCS_UCB_Slayer_" + profile.replace(" ", "_"), serial)
        weapon_type = self._detect_assigned_weapon_type(serial)
        self.slayer_weapon_types[profile] = weapon_type
        try:
            API.SavePersistentVar("JCS_UCB_SlayerType_" + profile.replace(" ", "_"), weapon_type, API.PersistentVar.Char)
        except Exception:
            pass
        API.SysMsg(profile + (" whip saved (AOE-capable)." if weapon_type == "bladedwhip" else " broadsword saved." if weapon_type == "broadsword" else " weapon saved."), 68)
        self.build_ui()
    ...
    def learn_current_mob(self, profile):
        if not self.current_enemy_serial:
            ...
            return
        self._save_learning(self.current_enemy_name, self.current_enemy_graphic, profile)
        self.detected_profile = profile
        self.classification_source = "Learned"
        API.SysMsg("Learned {} = {}.".format(self.current_enemy_name, profile), 68)
        self._apply_combat_weapon()
        self.build_ui()
    ...
    def target_enemy(self):
        ...
        target = API.RequestTarget()
        mob = self._find_mobile(self._serial_of(target))
        if mob:
    ...
    def _find_item(self, serial):
        ...
        try: return API.FindItem(serial)
        except Exception: return None

    def _find_mobile(self, serial):
        if not serial: return None
        try: return API.FindMobile(serial)
        except Exception: return None

    def _valid_enemy(self, mobile):
        if not mobile: return False
        try:
            if mobile.IsDead: return False
        except Exception: pass
        try:
            if API.IsFriend(mobile.Serial): return False
        except Exception: pass
        name = str(mobile.Name or "").strip().lower()
        if name in [entry.lower() for entry in MOBS_TO_IGNORE]: return False
        if name in [entry.lower() for entry in SUMMONS_TO_IGNORE]: return False
        return True

    def enemies(self, distance):
        try: mobiles = API.NearestMobiles(HOSTILE_NOTORIETIES, distance)
        except Exception: return []
        return [m for m in list(mobiles or []) if self._valid_enemy(m)]
    ...
    def _set_current_enemy(self, enemy):
        ...
        try: self.current_enemy_graphic = int(enemy.Graphic)
        except Exception: self.current_enemy_graphic = 0
        self._classify_current_enemy()
    ...
    def _classify_current_enemy(self):
        ...
        if learned_graphic:
            self.detected_profile, self.classification_source = learned_graphic, "Learned Body"
            return
        learned_name = self._load_learned_name(name)
        if learned_name:
            self.detected_profile, self.classification_source = learned_name, "Learned Name"
            return
        known = KNOWN_MOBS.get(name)
        if known:
            self.detected_profile, self.classification_source = known, "Known"
            return
        for profile in ("Poison Elemental", "Dragon", "Undead", "Demon", "Arachnid", "Reptile", "Elemental", "Fey", "Repond"):
            for keyword in SLAYER_KEYWORDS.get(profile, []):
                if keyword in name:
                    self.detected_profile, self.classification_source = profile, "Keyword"
                    return
        self.detected_profile, self.classification_source = "General Single Target", "Fallback"

    def _refresh_enchanted_apples(self, force=False):
        now = time.time()
        if not force and now < self.next_apple_check: return self.enchanted_apple_count
        self.next_apple_check = now + 3.0
        total = first_serial = first_graphic = 0
        try: items = API.ItemsInContainer(API.Backpack, True)
        except Exception: items = []
        for item in list(items or []):
            try: name = str(item.Name or "").strip().lower()
            except Exception: name = ""
            if "enchanted apple" not in name:
                try: props = API.ItemNameAndProps(self._serial_of(item), False, 1) or ""
                except Exception: props = ""
                name = str(props).replace("\r", "").split("\n")[0].strip().lower()
            if "enchanted apple" not in name: continue
            try: amount = int(item.Amount or 1)
            except Exception: amount = 1
            total += max(1, amount)
            if not first_serial:
                first_serial = self._serial_of(item)
                try: first_graphic = int(item.Graphic or 0)
                except Exception: pass
        self.enchanted_apple_serial, self.enchanted_apple_graphic, self.enchanted_apple_count = first_serial, first_graphic, total
        return total

    def use_enchanted_apple(self):
        if self._refresh_enchanted_apples(True) <= 0 or not self.enchanted_apple_serial:
    ...
    def _equipped_items(self):
        ...
        for layer in EQUIPMENT_LAYERS:
            try: item = API.FindLayer(layer)
            except Exception: item = None
            if not item: continue
            serial = self._serial_of(item)
            if serial and serial not in seen:
                seen.add(serial); found.append(item)
        return found

    def _item_durability(self, item):
        serial = self._serial_of(item)
        if not serial: return None
        try: props = API.ItemNameAndProps(serial, False, 1) or ""
        except Exception: props = ""
        if not props:
            try: props = API.ItemNameAndProps(serial, True, 1) or ""
            except Exception: props = ""
        for line in str(props).replace("\r", "").split("\n"):
            if "durability" not in line.lower(): continue
    ...
    def _lowest_durability_graphic(self):
        ...
        for item in self._equipped_items():
            ...
            if not dura: continue
            current, maximum = dura
            percent = 100.0 * current / maximum
            try: graphic = int(item.Graphic or 0)
            except Exception: graphic = 0
            if graphic and (worst is None or percent < worst[0]): worst = (percent, graphic)
    ...
    def check_durability(self, force_message=False):
        ...
        for item in self._equipped_items():
            ...
            try: name = str(item.Name or "Gear")
            except Exception: name = "Gear"
            entry = (self._serial_of(item), name, current, maximum, percent)
            checked.append(entry)
            if percent <= self.durability_threshold: low.append(entry)
        ...
        if force_message:
            if checked:
                ...
                API.SysMsg("Lowest durability: {} - {}/{} ({:.0f}%).".format(worst_all[1], worst_all[2], worst_all[3], worst_all[4]), 33 if worst_all[4] <= self.durability_threshold else 68)
                API.SysMsg("%d equipped item(s) are at or below the %d%% warning level." % (len(low), self.durability_threshold) if low else "All checked gear is above the %d%% warning level." % self.durability_threshold, 33 if low else 68)
            else:
                API.SysMsg("No equipped durability values could be read.", 33)
            return low
        now = time.time()
        if low and (newly_low or now >= self.next_durability_warning):
            self.next_durability_warning = now + 30.0
            worst = min(low, key=lambda entry: entry[4])
            API.SysMsg("DURABILITY WARNING: {} is {}/{} ({:.0f}%).".format(worst[1], worst[2], worst[3], worst[4]), 33)
            if len(low) > 1: API.SysMsg("%d equipped items are at or below %d%% durability." % (len(low), self.durability_threshold), 33)
        return low

    def _gold_items(self, recursive=True):
        try: items = API.ItemsInContainer(API.Backpack, bool(recursive))
        except Exception: items = []
        result = []
        for item in list(items or []):
            try:
                if int(item.Graphic) == GOLD_GRAPHIC and int(item.Amount or 0) > 0: result.append(item)
            except Exception: pass
        return result

    def _gold_in_backpack_root(self):
        try: items = API.ItemsInContainer(API.Backpack, False)
        except Exception: items = []
        result = []
        for item in list(items or []):
            try:
    ...
    def _combine_gold_to_root(self):
        ...
        for item in gold:
            serial = self._serial_of(item)
            if not serial or serial in root_serials: continue
            try: amount = int(item.Amount or 0)
            except Exception: amount = 0
            try:
                API.MoveItem(serial, API.Backpack, amount if amount > 0 else 0); API.Pause(0.35)
            except Exception: pass
        ...
        for item in list(root):
            ...
            try:
                API.MoveItem(serial, API.Backpack, amount); API.Pause(0.20)
            except Exception: pass
        return self._gold_in_backpack_root()

    def send_gold(self):
        if not self.send_bag_serial:
            API.SysMsg("Set your Bag of Sending in Setup first.", 33); return
        if not self._find_item(self.send_bag_serial):
            API.SysMsg("Saved Bag of Sending is not available.", 33); return
        stacks = self._combine_gold_to_root()
        if not stacks:
            API.SysMsg("No gold found in your backpack.", 68); return
        stacks = sorted(stacks, key=lambda item: int(getattr(item, "Amount", 0) or 0), reverse=True)
        sent = 0
        for item in stacks:
            serial = self._serial_of(item)
            if not serial or not self._find_item(serial): continue
            try: amount = int(item.Amount or 0)
            except Exception: amount = 0
            if amount > GOLD_MAX_STACK:
                API.SysMsg("Gold stack exceeds 60,000; leaving it unsent for safety.", 33); continue
            try:
                API.UseObject(self.send_bag_serial)
                if API.WaitForTarget("any", 1.5):
                    API.Target(serial); sent += 1; API.Pause(0.60)
                else:
                    API.SysMsg("Bag of Sending did not give a target cursor.", 33); break
            except Exception:
                API.SysMsg("Could not send gold.", 33); break
        if sent:
            API.Pause(0.25)
            charges = self._get_send_bag_charges(True)
            API.SysMsg("Sent %d gold stack(s) to the bank." % sent if charges is None else "Sent %d gold stack(s). Bag of Sending: %d charge(s) left." % (sent, charges), 68)
            self.refresh_combat_ui()
    ...
    def _hand_items(self):
        ...
        for layer in ("OneHanded", "TwoHanded"):
            ...
            if item:
                serial = self._serial_of(item)
                if serial and all(self._serial_of(existing) != serial for existing in result): result.append(item)
        return result

    def _shield_equipped(self):
        return bool(self.shield_serial and any(self._serial_of(item) == self.shield_serial for item in self._hand_items()))

    def _ensure_shield(self):
        if not self.shield_serial: return True
        if self._shield_equipped(): return True
        if not self._find_item(self.shield_serial): return False
        try:
            API.EquipItem(self.shield_serial); API.Pause(0.35)
        except Exception:
            return False
    ...
    def _equipped_assigned_weapon(self):
        ...
        for item in self._hand_items():
            if self._serial_of(item) in assigned: return item
        return None
    ...
    def _equip_profile(self, requested):
        ...
        if current:
            ...
            if current_serial and current_serial != serial and self.weapon_bag:
                try:
                    API.MoveItem(current_serial, self.weapon_bag, 1); API.Pause(0.40)
                except Exception: return False
        if not self._find_item(serial): return False
        try:
            API.MoveItem(serial, backpack.Serial, 1); API.Pause(0.35); API.EquipItem(serial); API.Pause(0.45)
        except Exception: return False
        self._ensure_shield()
        if self._equipped_assigned_serial() != serial: return False
    ...
    def _matching_slayer_is_aoe_capable(self):
        ...
        if not weapon_type:
            ...
            if weapon_type:
                self.slayer_weapon_types[profile] = weapon_type
                try: API.SavePersistentVar("JCS_UCB_SlayerType_" + profile.replace(" ", "_"), weapon_type, API.PersistentVar.Char)
                except Exception: pass
        return weapon_type == "bladedwhip"

    def _apply_combat_weapon(self):
        if self.manual_aoe: self._equip_profile("General AOE")
        elif self.auto_aoe_active:
            self._equip_profile(self.detected_profile if self._matching_slayer_is_aoe_capable() else "General AOE")
        else: self._equip_profile(self.detected_profile)

    def _update_auto_aoe(self):
        if not self.settings["auto_aoe"] or self.manual_aoe:
            self.auto_aoe_active = False; return
        count = len(self.enemies(AUTO_AOE_RANGE))
        if not self.auto_aoe_active and count >= AUTO_AOE_ENTER_COUNT:
            self.auto_aoe_active = True
            self._equip_profile(self.detected_profile if self._matching_slayer_is_aoe_capable() else "General AOE")
            self.refresh_combat_ui(); return
        if self.auto_aoe_active and count <= AUTO_AOE_EXIT_COUNT:
            self.auto_aoe_active = False; self._equip_profile(self.detected_profile); self.refresh_combat_ui(); return
        if self.auto_aoe_active:
            self._equip_profile(self.detected_profile if self._matching_slayer_is_aoe_capable() else "General AOE")

    def _ready(self, action, seconds):
        now = time.time()
        if now < self.next_actions.get(action, 0.0): return False
    ...
    def _on_buff_added(self, buff):
        try:
            key = self._buff_key(getattr(buff, "Title", ""))
            if key: self.active_buff_keys.add(key)
        except Exception: pass

    def _on_buff_removed(self, buff):
        try:
            key = self._buff_key(getattr(buff, "Title", ""))
            if key: self.active_buff_keys.discard(key)
        except Exception: pass

    def _register_buff_events(self):
        if self.buff_events_registered: return
        try:
    ...
    def _buff_active(self, wanted):
        ...
        if not wanted_key: return False
        for key in self.active_buff_keys:
            if wanted_key in key or key in wanted_key: return True
        if self._sync_active_buffs():
            for key in self.active_buff_keys:
                if wanted_key in key or key in wanted_key: return True
        try: return API.BuffExists(wanted)
        except Exception: return False
    ...
    def detect_weapon(self):
        if not self.settings["auto_weapon"] or time.time() < self.next_weapon_scan: return
        self.next_weapon_scan = time.time() + 0.50
        try:
            abilities = frozenset(self._ability_key(name) for name in API.CurrentAbilityNames() if name)
        except Exception: return
        pairs = {
        ...
        if detected == "ranged": detected = self.weapon_type if self.weapon_type in ("compositebow", "soulglaive") else "compositebow"
        if detected:
            self.weapon_type = detected
            self._save_settings()

    def _cast(self, spell, mana, cooldown, buff=None):
        if self._mana() < mana: return False
        if buff and self._buff_active(buff): return False
        if not self._ready("spell_" + spell, cooldown): return False
        API.CastSpell(spell)
        return True

    def _honor(self, enemy):
        if not self.settings["honor"]: return
        try: full_health = int(enemy.Hits or 0) == int(enemy.HitsMax or 0)
        except Exception: full_health = False
        if not full_health or not self._ready("honor_" + str(enemy.Serial), 8.0): return
        API.Virtue("honor")
        if API.WaitForTarget("any", 0.75): API.Target(enemy.Serial)

    def _enemy_type_key(self, enemy):
        if not enemy: return None
        try: graphic = int(enemy.Graphic or 0)
        except Exception: graphic = 0
        if graphic: return "body:{:04X}".format(graphic)
        try: name = str(enemy.Name or "").strip().lower()
        except Exception: name = ""
        return "name:" + "".join(ch for ch in name if ch.isalnum() or ch == " ") if name else None

    def _manage_enemy_of_one(self, enemy):
        if not self.settings["eoo"]:
            self.eoo_enemy_key = None; return
        if self._mana() < 12: return
        new_key = self._enemy_type_key(enemy)
        if not new_key: return
        active = self._buff_active("Enemy Of One")
        if active and self.eoo_enemy_key == new_key: return
        if active and self.eoo_enemy_key and self.eoo_enemy_key != new_key:
            try:
                API.CastSpell("Enemy of One"); API.Pause(1.0)
            except Exception: return
        if self._mana() >= 12 and not self._buff_active("Enemy Of One"):
            try:
                API.CastSpell("Enemy of One"); API.Pause(0.6)
            except Exception: return
        self.eoo_enemy_key = new_key

    def _new_target(self, enemy):
        if self.current_enemy_serial == int(enemy.Serial): return
    ...
    def _basher_ready(self):
        if not self.settings.get("bash", False) or not self.shield_serial or not self._shield_equipped(): return False
        try:
            parry = API.GetSkill("Parrying")
            if not parry or float(parry.Value) < 90.0: return False
        except Exception: return False
        return True

    def _shield_bash(self):
        if not self._basher_ready() or self._mana() < 40: return False
        if self._buff_active("Shield Bash") or not self._ready("mastery_shield_bash", 3.0): return False
        try:
            API.CastSpell("Shield Bash"); return True
        except Exception: return False

    def _buffs(self):
        self._shield_bash()
        if self.settings["curse"] and self._hits_percent() < 60: self._cast("Curse Weapon", 7, 1.25, "Curse Weapon")
        if self.settings["df"]: self._cast("Divine Fury", 8, 1.25, "Divine Fury")
        if self.settings["cw"]: self._cast("Consecrate Weapon", 6, 1.25, "Consecrate Weapon")
        if self.settings["ca"] and self._hits_percent() >= 60 and not self._buff_active("Confidence") and not self._buff_active("Evasion"):
    ...
    def _activate_secondary(self):
        if not API.SecondaryAbilityActive(): API.ToggleAbility("secondary")

    def _clear_weapon_abilities(self):
        if API.PrimaryAbilityActive(): API.ToggleAbility("primary")
        if API.SecondaryAbilityActive(): API.ToggleAbility("secondary")

    def _single_target_ability(self):
        if self.settings["onslaught"] and self._mana() >= 20:
            if self._cast("Onslaught", 20, 7.0, "Onslaught"):
                self._clear_weapon_abilities(); return
        if self.weapon_type == "broadsword":
            self._activate_secondary() if self._mana() >= 30 else self._clear_weapon_abilities(); return
        if self.weapon_type == "radiantscimitar":
            self._activate_secondary() if self._mana() >= 15 else self._clear_weapon_abilities(); return
        if self.weapon_type in ("longsword", "compositebow", "soulglaive", "doubleaxe", "bladedwhip"):
            self._activate_primary() if self._mana() >= 30 else self._clear_weapon_abilities(); return
        if self._mana() >= 30: self._activate_primary()
        elif self.settings["lightning"] and self._mana() >= 10:
            self._clear_weapon_abilities(); self._cast("Lightning Strike", 10, 1.0, "Lightning Strike")
        else: self._clear_weapon_abilities()

    def _multiple_target_ability(self):
        if self.weapon_type == "radiantscimitar":
            self._activate_primary() if self._mana() >= 15 else self._clear_weapon_abilities(); return
        if self.weapon_type in ("bladedwhip", "doubleaxe"):
            self._activate_secondary() if self._mana() >= 15 else self._clear_weapon_abilities(); return
        if self.settings["momentum"] and self._mana() >= 10:
            self._clear_weapon_abilities(); self._cast("Momentum Strike", 10, 1.0, "Momentum Strike"); return
    ...
    def combat_tick(self):
        ...
        if not victims:
            if self.current_enemy_serial:
                self._set_current_enemy(None); self.refresh_combat_ui()
            return
        enemy = victims[0]
        ...
        self._multiple_target_ability() if nearby >= 2 else self._single_target_ability()

    def run(self):
        self._register_buff_events()
        self._ensure_shield()
        self.build_ui()
        API.SysMsg("J.C.S. Ultimate Combat Bar v{} loaded - PUBLIC TEST.".format(VERSION), 68)
        API.SysMsg("Basher / Shield Bash support is BETA and needs Parry Mastery community testing.", 53)
        try:
            while self.running and not API.StopRequested:
                API.ProcessCallbacks()
                now = time.time()
                if now >= self.next_position_save:
                    self._remember_position(); self.next_position_save = now + 0.75
                if now >= self.next_shield_check:
                    self._ensure_shield(); self.next_shield_check = now + 0.75
                if now >= self.next_durability_check:
                    ...
                    if before != after and not self.setup_mode: self.refresh_combat_ui()
                if self.send_bag_serial and now >= self.next_send_bag_check:
                    ...
                    if old_charges != new_charges and not self.setup_mode: self.refresh_combat_ui()
                if now >= self.next_apple_check:
                    old_apples = self.enchanted_apple_count
                    new_apples = self._refresh_enchanted_apples(True)
                    if old_apples != new_apples and not self.setup_mode: self.refresh_combat_ui()
                if not self.paused and not API.Player.IsDead:
                    self.detect_weapon(); self.combat_tick()
                API.Pause(0.10)
        finally:
            self._remember_position()
            self._save_settings()
